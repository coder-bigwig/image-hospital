 ...
 * 手动释放变量内存（C++申请内存）
 */
dwv.image.View.prototype.free = function()
{
    this.getImage().free();
};
...
{        
    ...
    case "MONOCHROME1":
        ...
        {
            pxValue = 255-parseInt( windowLut.getValue(
...
dwv.image.View.prototype.generateImageDataHexa = function( array, canvasWidth, canvasHieght )
{
    var sliceNumber = this.getCurrentPosition().k;    
    var image = this.getImage();    
    var pxValue = 0;
    ...
    var index = 0;
    var sliceSize = image.getGeometry().getSize().getSliceSize();
    var iWidth = image.getGeometry().getSize().getNumberOfColumns();
    var iHeight = image.getGeometry().getSize().getNumberOfRows();
    var sliceOffset = 0;
    switch (photoInterpretation)
    {
    case "MONOCHROME1":
        sliceOffset = (sliceNumber || 0) * sliceSize;
        var iMax = sliceOffset + sliceSize;
        Module.setWindowWC(image.getBuffer(), iWidth, iHeight, canvasWidth, canvasHieght, 
            image.getMeta().pixelRepresentation, sliceOffset, iMax, array.data, 0, 
            colourMap.red, colourMap.green, colourMap.blue, windowLut.getBuffer(), 1);
        break;
    ...
    case "MONOCHROME2":
        ...
        Module.setWindowWC(image.getBuffer(), iWidth, iHeight, canvasWidth, canvasHieght, 
            image.getMeta().pixelRepresentation, sliceOffset, iMax, array.data, 0, 
            colourMap.red, colourMap.green, colourMap.blue, windowLut.getBuffer(), 0);
        break;
...
dwv.image.ViewFactory.prototype.create = function (dicomElements, pixelBuffer)
{
    ...
    if ( windowPresets.length !== 0 && windowPresets[0].width>1) {
