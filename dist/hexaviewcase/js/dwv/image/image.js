...
dwv.image.ImageFactory.prototype.create = function (dicomElements, pixelBuffer)
{
    ...
    // buffer data
    /////////////////////////// TODO 使用c++申请内存，需要手动释放
    var pixelRepresentation = dicomElements.getFromKey("x00280103");
    var buffer = null;
    if (pixelBuffer.BYTES_PER_ELEMENT===2){
        if ( pixelRepresentation === 1 ) {
            // unsigned to signed data
            buffer = _mallocHeap16BySize(pixelBuffer.length, 1);
            ...
            buffer = _arrayTo16Heap(pixelBuffer, 0);
     }
    else{
        buffer = _arrayToHeap2d(pixelBuffer);
    }
    ...
    // meta information
    var meta = {};
    meta.pixelRepresentation = pixelRepresentation;
    // Modality
